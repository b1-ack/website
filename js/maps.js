...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.map) {
        ...
        if (mapContainer) {
            mapContainer.addEventListener('touchstart', function() {
                ...
                var layersExpanded = layersCtrl && layersCtrl.classList.contains('leaflet-control-layers-expanded');
                var poiExpanded = poiDropdown && poiDropdown.classList.contains('active');
                if (layersExpanded || poiExpanded || document.activeElement && document.activeElement.closest('.leaflet-control-layers')) {
                    mapClickShouldBlur = true;
                }
        ...
        window.map.on('mousedown', function() {
            ...
            var layersCtrl = document.querySelector('.leaflet-control-layers');
            if (layersCtrl && layersCtrl.classList.contains('leaflet-control-layers-expanded')) {
                anyFocused = true;
            }
            var poiExpanded = poiDropdown && poiDropdown.classList.contains('active');
            if (poiExpanded) anyFocused = true;
            if (anyFocused) mapClickShouldBlur = true;
        });
    ...
    var poiCategories = [
        { id: 'pharmacy', label: 'Аптека', query: 'node["amenity"="pharmacy"];way["amenity"="pharmacy"];' },
        { id: 'food', label: 'Еда', query: 'node["amenity"~"restaurant|cafe|fast_food"];way["amenity"~"restaurant|cafe|fast_food"];' },
        { id: 'hotels', label: 'Гостиницы', query: 'node["tourism"~"hotel|motel|hostel"];way["tourism"~"hotel|motel|hostel"];' },
        { id: 'entertainment', label: 'Развлечения', query: 'node["amenity"~"cinema|theatre|nightclub"];node["tourism"="attraction"];node["leisure"="park"];way["amenity"~"cinema|theatre|nightclub"];way["tourism"="attraction"];way["leisure"="park"];' },
        { id: 'atms', label: 'Банкоматы', query: 'node["amenity"="atm"];node["amenity"="bank"];way["amenity"="atm"];way["amenity"="bank"];' },
        { id: 'groceries', label: 'Продукты', query: 'node["shop"~"supermarket|convenience"];way["shop"~"supermarket|convenience"];' },
        { id: 'fuel', label: 'АЗС', query: 'node["amenity"="fuel"];way["amenity"="fuel"];' }
    ];

    var poiLayer = L.layerGroup().addTo(window.map);
    var activeCategory = null;
    var poiIcons = {};
    var poiLoading = false;

    function makePoiIcon(categoryId) {
        return L.divIcon({
            className: 'poi-marker',
            html: '<div class="poi-marker-inner" data-category="' + categoryId + '"></div>',
            iconSize: [24, 24],
            iconAnchor: [12, 12]
        });
    }
    ...
    function fetchPoiByCategory(categoryId) {
        if (!window.map) return;
        var cat = null;
        for (var i = 0; i < poiCategories.length; i++) {
            if (poiCategories[i].id === categoryId) { cat = poiCategories[i]; break; }
        }
        if (!cat) return;
        ...
        var overpassQuery = '[out:json][timeout:15];(' + cat.query + ');out center;';
        ...
        fetch(url)
            ...
            .then(function(data) {
                ...
                var added = 0;
                for (var i = 0; i < data.elements.length && added < 100; i++) {
                    var el = data.elements[i];
                    var lat = el.lat || (el.center && el.center.lat);
                    var lon = el.lon || (el.center && el.center.lon);
                    if (lat == null || lon == null) continue;
                    var name = el.tags && (el.tags.name || el.tags.brand || el.tags.operator || el.tags.shop || el.tags.amenity);
                    if (!name) continue;
                    var icon = makePoiIcon(categoryId);
                    var marker = L.marker([lat, lon], { icon: icon }).addTo(poiLayer);
                    var popupContent = '<div style="font-weight:600;font-size:14px;margin-bottom:4px">' + name + '</div>';
                    if (el.tags && el.tags['addr:street']) {
                        popupContent += '<div style="font-size:12px;opacity:0.6">' + el.tags['addr:street'];
                        if (el.tags['addr:housenumber']) popupContent += ' ' + el.tags['addr:housenumber'];
                        popupContent += '</div>';
                    }
                    marker.bindPopup(popupContent);
                    poiIcons[added] = marker;
                    added++;
                }
            })
            .catch(function() { poiLoading = false; });
    }

    var poiBtns = document.querySelectorAll('.poi-btn[data-category]');
    var poiDropdownList = document.getElementById('poi-dropdown-list');
    var poiDropdown = document.getElementById('poi-dropdown');
    var poiMoreToggle = document.getElementById('poi-more-toggle');

    function setActiveCategory(categoryId) {
        if (activeCategory === categoryId) {
            activeCategory = null;
            poiBtns.forEach(function(b) { b.classList.remove('active'); });
            var items = poiDropdownList.querySelectorAll('.poi-dropdown-item');
            items.forEach(function(it) {
                it.classList.remove('active');
                var r = it.querySelector('input[type="radio"]');
        ...
        poiBtns.forEach(function(b) {
            b.classList.toggle('active', b.dataset.category === categoryId);
        });
        var items = poiDropdownList.querySelectorAll('.poi-dropdown-item');
        items.forEach(function(it) {
            var isActive = it.dataset.category === categoryId;
            it.classList.toggle('active', isActive);
            var r = it.querySelector('input[type="radio"]');
            if (r) r.checked = isActive;
        });
        fetchPoiByCategory(categoryId);
    }
    ...
    var layerSvgs = {
        pharmacy: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="4" width="16" height="16" rx="2"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>',
        food: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 2v4M12 2v4M6 2v4M3 10h18M5 10v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V10"/><path d="M9 14h1"/><path d="M14 14h1"/></svg>',
        hotels: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21V7a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v14"/><path d="M7 3v18"/><rect x="8" y="10" width="8" height="4" rx="1"/><path d="M8 18h8"/></svg>',
        entertainment: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4a7 7 0 0 0-7 7c0 3.5 2.5 6.5 7 9 4.5-2.5 7-5.5 7-9a7 7 0 0 0-7-7z"/><path d="M12 19v5"/><path d="M9 24h6"/></svg>',
        atms: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/><circle cx="12" cy="15" r="2"/><path d="M6 6V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v2"/></svg>',
        groceries: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>',
        fuel: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 22V4a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v18"/><path d="M13 22h8"/><rect x="5" y="8" width="6" height="4"/><path d="M17 22V10a2 2 0 0 1 2-2h0a2 2 0 0 1 2 2v4"/><circle cx="19" cy="18" r="1"/></svg>'
    };

    poiCategories.forEach(function(cat) {
        var label = document.createElement('label');
        label.className = 'poi-dropdown-item';
        label.dataset.category = cat.id;
        var radio = document.createElement('input');
        radio.type = 'radio';
        radio.name = 'poi-category';
        label.appendChild(radio);
        label.innerHTML += layerSvgs[cat.id] + '<span class="poi-dropdown-name">' + cat.label + '</span>';
        label.addEventListener('click', function(e) {
            e.stopPropagation();
            if (poiDropdown) poiDropdown.classList.remove('active');
            if (poiMoreToggle) poiMoreToggle.classList.remove('active');
            setActiveCategory(cat.id);
        });
        poiDropdownList.appendChild(label);
    });

    if (poiMoreToggle && poiDropdown) {
        poiMoreToggle.addEventListener('click', function(e) {
            e.stopPropagation();
            var layersCtrl = document.querySelector('.leaflet-control-layers');
            if (layersCtrl) layersCtrl.classList.remove('leaflet-control-layers-expanded');
            poiMoreToggle.classList.toggle('active');
            poiDropdown.classList.toggle('active');
        });
        document.addEventListener('click', function() {
            poiDropdown.classList.remove('active');
            poiMoreToggle.classList.remove('active');
        });
        poiDropdown.addEventListener('click', function(e) {
            e.stopPropagation();
        });
    }

    var layersToggleEl = document.querySelector('.leaflet-control-layers-toggle');
    if (layersToggleEl) {
        layersToggleEl.addEventListener('click', function() {
            if (poiDropdown) poiDropdown.classList.remove('active');
            if (poiMoreToggle) poiMoreToggle.classList.remove('active');
        });
    }
    ...
    window.map.on('moveend', function() {
        if (activeCategory) {
            fetchPoiByCategory(activeCategory);
        }
    });

    if (window.innerWidth < 768) {
        var poiOverlays = {};
        poiCategories.forEach(function(c) {
            poiOverlays[c.label] = L.layerGroup();
        });
        var poiCtrl = L.control.layers(null, poiOverlays, { position: 'bottomleft' }).addTo(window.map);
        L.DomUtil.addClass(poiCtrl.getContainer(), 'poi-layer-control');
        map.on('overlayadd', function(e) {
            var found = null;
            for (var i = 0; i < poiCategories.length; i++) {
                if (poiCategories[i].label === e.name) { found = poiCategories[i]; break; }
            }
            if (!found) return;
            for (var key in poiOverlays) {
                if (key !== e.name && window.map.hasLayer(poiOverlays[key])) {
                    window.map.removeLayer(poiOverlays[key]);
                }
            }
            activeCategory = found.id;
            fetchPoiByCategory(found.id);
        });
